...
{
    public class EnhancedMonstersTraceScript : MonoBehaviour
    {
    ...
    public class EnhancedMonstersPatch()
    {
        ...
        [HarmonyPatch(typeof(EnemyAI_Patches), "KillEnemy")]
        [HarmonyTranspiler]
        ...
        [HarmonyPatch(typeof(EnemiesDataManager), "EnsureEnemy2PropPrefabs")]
        [HarmonyTranspiler]
        static IEnumerable<CodeInstruction> EM_EnsureEnemy2PropPrefabsPostfixT(IEnumerable<CodeInstruction> instructions)
        {

            Script.Logger.LogWarning("Fired Transpiller for EnsureEnemy2PropPrefabs");
            CodeMatcher matcher = new CodeMatcher(instructions);
            ...
            matcher.MatchForward(false,
                ...
                new CodeMatch(OpCodes.Ldloc_S),
                new CodeMatch(OpCodes.Call)
                ).ThrowIfInvalid("Could not find match for EnsureEnemy2PropPrefabs")
                .InsertAndAdvance(new CodeInstruction(OpCodes.Ldloc_S))
                .Insert(new CodeInstruction(OpCodes.Callvirt, AccessTools.Method(typeof(GameObject), nameof(GameObject.AddComponent), generics: [typeof(EnhancedMonstersTraceScript)])));
                ;
